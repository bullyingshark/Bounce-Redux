...
class Checkpoint:
    ...
    def draw(self, surface, camera_x, camera_y=0):
        ...
        if (0 <= screen_x <= surface.get_width() + self.size and
                0 <= screen_y <= surface.get_height() + self.size):
            ...
            if image:
                ...
                surface.blit(scaled_image, (screen_x - self.size // 2, screen_y - self.size // 2))
            else:
                ...
                pygame.draw.polygon(
                    ...
                    flag_color,
                    [
                        (screen_x - self.size // 4, screen_y - self.size // 3),
                        (screen_x + self.size // 4, screen_y - self.size // 4),
                        (screen_x - self.size // 4, screen_y - self.size // 6)
                    ]
                )
                # Draw the pole
                pygame.draw.rect(
                    surface,
                    (100, 100, 100),
                    (screen_x - self.size // 4, screen_y - self.size // 2, 4, self.size)
                )