...
FPS = 60
TILE_SIZE = 56
# Add dimensions for coins and enemies
# Replace COIN_SIZE with separate width and height parameters
...
JUMP_POWER = -13
MOVE_SPEED = 5
# Updating the threshold values for horizontal and vertical scrolling
...
# A function for uploading images
def load_image(name, fallback_color=None):
    try:
        if os.path.exists(name):
            ...
            print(f"Image file not found: {name}")
    except pygame.error as e:
        print(f"Failed to load the image: {name}")
        print(f"Error: {e}")
    ...
    if fallback_color:
        ...
        # A chequered pattern as a filler
...
# Use the larger dimension to support vertical scrolling
...
heart_image = load_image("img/gbar_life@2x.png", RED)  # Images for the UI life
coin_ui_image = load_image("img/gbar_ring@2x.png", RED)  # Images for displaying UI coins
# Add an image for the life bonus
life_bonus_image = load_image("img/life@2x.png", GREEN)  # using red as a placeholder
